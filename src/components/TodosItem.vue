<template>
	<li
		...
		:key="index"
		class="even:border-y border-violet px-4 py-6 flex items-center justify-between"
	>
		<div class="flex items-center space-x-2">
			<input
				class="accent-violet p-1 outline-none w-6 h-6 rounded-sm border border-violet peer"
				type="checkbox"
				v-bind:checked="value.completed"
				:id="'todo-' + value.id"
			/>
			<label
				:for="'todo-' + value.id"
				class="font-medium text-xl leading-5 peer-checked:line-through"
			>
				{{ value.title }}
			</label>
		</div>
